...
router.post('/create', requireAuth, async (req, res) => {
    ...
    prizeDescription
     ...
     // helper to return HTML render or JSON error
      function respondError(msg) {
        return res.format({
          html: () => res.status(400).render('egaming/createTournament', { 
            layout:  'main',
            title:   'Create Tournament',
            games:    esportsGames,           
    ...
    for (const t of occupied) {
      ...
      const oStart = new Date(t.date); oStart.setHours(oh1, om1, 0, 0);
      const oEnd   = new Date(t.date); oEnd.setHours(oh2, om2, 0, 0);

      // intervals overlap?
      if (startDateTime < oEnd && oStart < endDateTime) {
        return respondError('You already have a tournament (hosted or joined) that overlaps this date & time.');
      }
    }
