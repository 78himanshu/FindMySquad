...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!sport || !allowedSports.includes(sport)) return showToastError("Please select a valid sport.");
    if (!gameDate || !startTime || !endTime) return showToastError("Game date and times are required.");
    ...
    if (start < new Date()) return showToastError("You cannot host a game in the past.");

    if (isNaN(playersRequired) || playersRequired < 1) return showToastError("Players Required must be a number greater than 0.");
    if (isNaN(costPerHead) || costPerHead < 0) return showToastError("Cost per Head must be a valid number (0 or more).");

    if (!skillLevel || !allowedSkills.includes(skillLevel)) return showToastError("Invalid skill level.");
    if (!location) return showToastError("Location is required.");
    if (!description) return showToastError("Description is required.");
    if (description.split(/\s+/).length > 250) return showToastError("Description cannot exceed 250 words.");

    const lowered = description.toLowerCase();
    for (let word of badWords) {
      if (lowered.includes(word)) return showToastError("Description contains inappropriate language.");
    }
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "Failed to save the game.");

      showToast("Game successfully " + (isEditMode ? "updated!" : "hosted!"), "success");
      setTimeout(() => {
